...
class CTATrace:
    def __init__(self, head_source_trace):
        # Номер трассы
        self.number = 0
        # Текущее время
        self.ticks = 0
    ...
    def get_all_source_traces(self):
    ...
    def must_identify_with_source_trace(self, trace: SourceTrace):
        all_self_source_traces = self.get_all_source_traces()
        return not any(trace.mfr_number == source_trace.mfr_number for source_trace in all_self_source_traces)

    # Проверяет нужно ли отождествление с этой трассой ЕМТ
    def must_identify_with_cta_trace(self, cta_trace):
        all_self_source_traces = self.get_all_source_traces()
        all_cta_trace_source_traces = cta_trace.get_all_source_traces()
        return not any(self_source_trace.mfr_number == cta_trace_source_trace.mfr_number
                       for self_source_trace in all_self_source_traces
                       for cta_trace_source_trace in all_cta_trace_source_traces)
    ...
    def sort_sources(self):
        ...
        # Формирование массива источников для данной трассы ЕМТ (для упрощения сортировки)
        all_source_traces = self.get_all_source_traces()
        # Сначала сортируем по признаку пеленга, потом по АС, далее по времени оценки координат
        all_source_traces.sort(key=sort_func, reverse=True)
        # Запоминаем головную трассу и дополнительные источники
        self.head_source_trace, *self.additional_source_trace_array = all_source_traces
    ...
    def delete_sources_traces(self):
        all_source_traces = self.get_all_source_traces()
        for source_trace in all_source_traces:
            source_trace.delete_cta_info_and_number()
